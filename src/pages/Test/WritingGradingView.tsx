...
import { Copy, Eye, EyeOff, ChevronLeft, ChevronRight } from 'lucide-react'
import { WritingResponse, GeneralStudentEssayResponse, StudentEssayResponse } from '../../store/slices/attemptSlice'
import { QuestionGroup } from '../../store/slices/testSlice'
import { getStudentsByTestId, getStudentEssayResponse } from '../../services/Attempt'
...
export const WritingGradingView: React.FC = () => {
  ...
  const [students, setStudents] = useState<GeneralStudentEssayResponse[]>([])
  const [selectedStudentIndex, setSelectedStudentIndex] = useState(0)
  const [currentResponse, setCurrentResponse] = useState<StudentEssayResponse | null>(null)
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      if (!testId) return
      try {
        const response = await getStudentsByTestId(testId)
        if (response.code === 1000 && response.result) {
          setStudents(response.result)
          if (response.result.length > 0) {
            fetchStudentResponse(response.result[0].attemptId)
          }
        }
      } catch (error) {
  ...
  const fetchStudentResponse = async (attemptId: string) => {
    setLoading(true)
    try {
      const response = await getStudentEssayResponse(attemptId)
      if (response.code === 1000 && response.result) {
  ...
  const handleStudentChange = (index: number) => {
    ...
    fetchStudentResponse(students[index].attemptId)
  }

  // Get section and questionGroup from the current response
  const section = currentResponse?.testAttempt?.test?.sections?.[0] // Assuming writing is in first section
  const questionGroup =
    section?.questionGroups?.find((group) => group.questionType === 'WRITING') || section?.questionGroups?.[0]
  const responsesToUse = currentResponse?.responses || []
    ...
    <div className='max-w-7xl mx-auto p-6 bg-gray-50 min-h-screen'>
      ...
      <div className='mb-6 bg-white rounded-lg shadow-sm border p-4'>
        <div className='flex items-center justify-between'>
          <button
            onClick={() => handleStudentChange(selectedStudentIndex - 1)}
          ...
          <div className='flex-1 text-center'>
            {students.length > 0 ? (
              <div>
                <h3 className='text-lg font-semibold text-gray-800'>{students[selectedStudentIndex].name}</h3>
                {/* <p className='text-sm text-gray-500'>Student ID: {students[selectedStudentIndex].id}</p> */}
              </div>
            ) : (
      ...
      {loading ? (
        ...
        <div className='flex justify-center items-center h-64'>
          <p className='text-gray-500'>No writing questions found for this test</p>
        </div>
      ) : (
        <div className='grid grid-cols-1 lg:grid-cols-3 gap-6'>
            ...
            <div className='bg-white rounded-lg shadow-sm border p-6 sticky top-6'>
              ...
              {questionGroup.instructions && (
                <div className='mb-4 p-3 bg-orange-50 border-l-4 border-orange-400 rounded-r-lg'>
                  ...
                  <p className='text-orange-700 text-sm whitespace-pre-wrap'>{questionGroup.instructions}</p>
                </div>
              )}
              ...
              {questionGroup.questions?.map((question) => (
                <div key={question.questionId} className='mb-4 p-4 bg-gray-50 rounded-lg border'>
                  <h3 className='font-medium text-gray-700 mb-2'>
                    {questionGroup.title || `Question ${question.questionNumber}`}
                  </h3>
                  ...
                  {question.maxWords && (
                    <div className='mt-2 text-xs text-blue-600 bg-blue-100 px-2 py-1 rounded inline-block'>
                      Minimum: {getMinWordRequirement(questionGroup)} words
                    </div>
                  )}
                </div>
              ))}
              ...
              {questionGroup.content && (
                <div className='mt-4 p-3 bg-gray-50 border border-gray-200 rounded'>
                  ...
                  <p className='text-sm text-gray-700 whitespace-pre-wrap'>{questionGroup.content}</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className='lg:col-span-2 space-y-6'>
            {responsesToUse.map((response) => {
              ...
              const minWords = getMinWordRequirement(questionGroup)
                        ...
                        <div>
                          ...
                          <div className='space-y-3'>
                            {[
                              ...
                              {
                                key: 'grammaticalRange' as keyof GradingCriteria,
                                label: 'Grammatical Range & Accuracy'
                              },
